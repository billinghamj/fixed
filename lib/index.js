...
fixed.prototype.generate = function (data) {
	var buffer = new Buffer(this.spec.length);
	buffer.fill(' ');

	this.spec.fields.forEach(function (field) {
	...
	if (this.spec.recordEnding) {
		var endLen = this.spec.recordEnding.length;
		buffer.write(this.spec.recordEnding, this.spec.length - endLen, endLen);
	}
...
function processSpec(spec) {
	...
	if (spec.recordEnding)
		spec.length += spec.recordEnding.length;
